   ...
   const handleDelete = async (id: string, title: string) => {
     ...
     try {
       ...
       setPagination(prev => ({ 
         ...
         total_pages: Math.max(1, Math.ceil((prev.total - 1) / ITEMS_PER_PAGE))
       }));
       setDeleteId(null);
     } catch (error: any) {
       console.error("Delete error:", error);
       const errMsg = error?.message || "Gagal menghapus properti";
       toast({ 
                ...
                {filtered.map(p => (
                  <tr key={p.id} className="hover:bg-gray-50/50 transition-colors">
                     <td className="px-4 py-3">
                       <div className="w-16 h-16 rounded-lg overflow-hidden border border-gray-100 bg-gray-50 flex items-center justify-center">
                         <img 
                           src={p.image || "https://via.placeholder.com/100?text=No+Image"} 
                           alt={p.title} 
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <button onClick={() => handleDuplicate(p)} className="p-1.5 text-gray-400 hover:text-green-500 hover:bg-green-50 rounded-lg transition-colors" title="Duplikasi">
                         ...
                         <button
                           ...
                           title={deleteId === p.id ? "Klik lagi untuk menghapus permanen" : "Hapus properti"}
                         >
