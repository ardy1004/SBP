...
import { transformSupabaseProperty, transformSupabaseProperties } from "@/lib/utils/property";
...
export default function PropertyDetailPage() {
  ...
  devLog.log('🚀 PropertyDetailPage COMPONENT RENDERED');

  const [location, setLocation] = useLocation();
  const params = useParams();

  devLog.log('📍 Location from useLocation:', location);
  devLog.log('📍 Params from useParams:', params);
  ...
  if (isDirectPropertyRoute) {
    ...
    devLog.log('🎯 Direct property route detected');
    kodeListing = params.id;
    propertyId = params.id;
    devLog.log('🏷️ Direct kode listing:', kodeListing);
  } else if (hasStructuredParams) {
    // New format: /{status}/{jenis}/{kabupaten}/{provinsi}/{judul}
    devLog.log('🆕 Using structured route parameters');
    ...
    if (potentialKodeListing && /^[A-Z]+\d+[\.\d]*$/.test(potentialKodeListing)) {
      ...
      devLog.log('🏷️ Kode listing extracted from structured params:', kodeListing);
      ...
      devLog.log('🔄 Reconstructing slug for parsing:', reconstructedSlug);
      const parsedSlug = parsePropertySlug(reconstructedSlug);
      kodeListing = parsedSlug.kode_listing;
      propertyId = kodeListing;
      devLog.log('🏷️ Kode listing from reconstructed slug:', kodeListing);
    }
  } else {
    // Legacy format: get slug from URL path (remove leading slash)
    const slug = location.substring(1);
    devLog.log('📝 Slug extracted (legacy):', slug);

    const parsedSlug = parsePropertySlug(slug);
    devLog.log('🔍 Parsed slug result (legacy):', parsedSlug);

    kodeListing = parsedSlug.kode_listing;
    devLog.log('🏷️ Kode listing from legacy slug:', kodeListing);
  ...
  devLog.log('🆔 Final propertyId used for query:', propertyId);

  const { favorites, addFavorite, removeFavorite } = usePropertyStore();
  
  // State for canonical URL - must be before any early returns
  ...
  const { data: property, isLoading } = useQuery<Property>({
    ...
    queryFn: async () => {
      ...
      devLog.log('=== PROPERTY DETAIL QUERY ===');
      devLog.log('Property ID/Kode Listing:', propertyId);
      devLog.log('Has structured params:', hasStructuredParams);
      devLog.log('Route params:', params);
      ...
      if (kodeListing) {
        devLog.log('Querying by kode_listing:', kodeListing);
        
        // First try case-insensitive exact match
        let { data, error } = await query.ilike('kode_listing', kodeListing).single();
        
        // If not found, try with wildcards for partial match
        if (error || !data) {
          devLog.log('Case-insensitive match failed, trying partial match...');
          const { data: partialData, error: partialError } = await query.ilike('kode_listing', `%${kodeListing}%`).single();
          if (!partialError && partialData) {
            devLog.log('Found property with partial match');
            return transformSupabaseProperty(partialData);
          }
        }
        
        if (error) {
          devLog.error('Supabase query error:', error);
          devLog.error('Error details:', error.message, error.details, error.hint);
          throw error;
        }
        
        if (!data) {
          devLog.error('No data returned from query');
          throw new Error('Property not found');
        }
        
        devLog.log('Raw Supabase data:', data);
        const transformedProperty = transformSupabaseProperty(data);
        devLog.log('Transformed property:', transformedProperty);
        return transformedProperty;
      } else {
        devLog.log('Querying by id:', propertyId);
        const { data, error } = await query.eq('id', propertyId).single();

        devLog.log('Query result - data:', data);
        devLog.log('Query result - error:', error);
    ...
    if (property) {
      ...
      const trackAnalytics = async () => {
        try {
          ...
          if (error) {
            devLog.error('Analytics tracking failed:', error);
          } else {
            devLog.log('Analytics event tracked');
          }
        } catch (error) {
          devLog.error('Analytics tracking error:', error);
        }
      };
  ...
  // Effect for generating canonical URL - must be before any early returns
  useEffect(() => {
    if (!property) return;
    
    const generateUrl = async () => {
      const origin = typeof window !== 'undefined' ? window.location.origin : 'https://salambumi.xyz';
  ...
  const handleInquirySubmit = async (data: { name: string; whatsapp: string; message: string }) => {
    ...
    if (error) {
      devLog.error('Inquiry submission error:', error);
  ...
  const getOptimizedImageUrl = () => {
    ...
    if (primaryImage.startsWith('http')) {
      // Add query params for better social media compatibility
      const url = new URL(primaryImage);
      // Remove any existing size params and add consistent sizing
  ...
  return (
    <>
      ...
      <Helmet>
        ...
        <meta property="og:updated_time" content={property.updatedAt?.toISOString() || new Date().toISOString()} />
        ...
        <meta name="twitter:image:alt" content={getTitle()} />
      </Helmet>
...
function RelatedPropertiesSection({ currentProperty }: { currentProperty: Property }) {
  ...
  const { data: relatedProperties = [], isLoading } = useQuery<{
    ...
    queryFn: async () => {
      ...
      return {
        locationBased: transformSupabaseProperties(locationBased),
        typeBased: transformSupabaseProperties(typeBased),
        recent: transformSupabaseProperties(recent)
      };
    },
  ...
  // transformSupabaseProperty dan transformSupabaseProperties now imported from @/lib/utils/property
