...
export function PropertyPilihanSlider({ properties }: PropertyPilihanSliderProps) {
  ...
  const handleViewDetails = () => {
    ...
    // Navigate using direct property route with kodeListing
    // This ensures it goes to the property detail page correctly
    if (currentProperty.kodeListing) {
      setLocation(`/properti/${currentProperty.kodeListing}`);
    } else if (currentProperty.id) {
      setLocation(`/properti/${currentProperty.id}`);
    } else {
      // Fallback to SEO-friendly slug for navigation
