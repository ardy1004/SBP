 ...
 * Now supports both images (converts to WebP) and PDFs (uploads directly)
 */
export const uploadFileToWorker = async (file: File, articleId?: string, retryCount = 0): Promise<string> => {
  const maxRetries = 3;
  ...
  try {
    ...
    if (!isPdf) {
      // For images: Convert to WebP first
      console.log('Converting image to WebP:', file.name);
      fileToUpload = await convertToWebP(file);
      console.log('WebP conversion successful, new file:', fileToUpload.name, 'size:', fileToUpload.size);
    } else {
      console.log('PDF file detected, uploading directly:', file.name, 'size:', file.size);
    }
    ...
    // For development: ALWAYS upload PDFs to cloud (don't use data URL)
    // Only use data URL fallback for images in development
    if (!isProduction && !isPdf) {
      // For development images only: Use data URL instead of uploading to avoid cloud dependency
      console.log('Development mode (image): Using data URL instead of cloud upload');
    ...
    // Development: Upload PDFs to cloud for proper URL
    const formData = new FormData();
    formData.append('image', fileToUpload);
    formData.append('propertyId', articleId || 'agreement-temp');

    // Use the worker URL that's in CSP - sbp-upload-worker.salambumiproperty-f1b.workers.dev
    const uploadUrl = 'https://sbp-upload-worker.salambumiproperty-f1b.workers.dev/upload';
    
    console.log(`Uploading ${isPdf ? 'PDF' : 'WebP'} file to Cloudflare Worker:`, uploadUrl);
    ...
    console.log(`${isPdf ? 'PDF' : 'WebP'} file URL from worker:`, fileUrl);
