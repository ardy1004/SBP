...
async function serveSPA(request, env) {
	try {
		...
		// Check if this might be a crawler trying to access a property page
		// If the URL looks like a property slug (has multiple dashes), try to handle it
		...
		// If it's a crawler and the path looks like a property slug (contains dashes)
		// Try to handle it as a slug redirect
		if (isCrawler && path.includes('-') && path.length > 10) {
			console.log('SPA fallback: Detected crawler with potential slug:', path);
			const slugResult = await handleSlugRedirect(request, env, url);
			if (slugResult) return slugResult;
		}
...
<head>
	...
	<meta name="description" content="Temukan properti impian Anda di Salam Bumi Property - Rumah, Kost, Apartemen, dan Tanah di Yogyakarta dan sekitarnya.">
	<script>
...
async function handlePropertyShare(request, env, url) {
	...
	try {
		...
		} else if (!mainImageUrl.match(/\?.*w=/)) {
			// Add dimensions if not present
			mainImageUrl += mainImageUrl.includes('?') ? '&' : '?';
			mainImageUrl += 'w=1200&h=630&fit=crop';
		}
		...
		const html = generateShareCardHTML(property, kodeListing, mainImageUrl, null);
...
async function handleSlugRedirect(request, env, url) {
	...
	try {
		...
		console.log('Extracted kode_listing:', kodeListing);
		...
		// Fetch property data
		console.log('Fetching property from Supabase with kode_listing:', kodeListing);
		const property = await fetchPropertyFromSupabase(kodeListing, env);
		...
		console.log('Property found:', property.kode_listing, '-', property.judul_properti);
		console.log('Images:', property.image_url, property.image_url1, property.image_url2);
		...
		console.log('Crawler patterns matched:', crawlerPatterns.filter(p => new RegExp(p, 'i').test(userAgent)));

		if (isCrawler) {
			console.log('Serving OG meta tags for crawler');
			console.log('Generating share card HTML...');
			...
			console.log('OG Image meta tag:', mainImageUrl);
			console.log('========================================');
			return new Response(html, {
				headers: {
					'Content-Type': 'text/html; charset=utf-8',
...
function generateShareCardHTML(property, propertyId, mainImageUrl, slug = null) {
	...
	// Use slug-based URL if available (SEO-friendly), otherwise fallback to /p/ format
...
<head>
	...
	<!-- Open Graph / Facebook - Optimized for WhatsApp -->
	...
	<meta property="og:updated_time" content="${property.updated_at || new Date().toISOString()}">
	...
	<!-- WhatsApp specific - Preconnect to image domain for faster loading -->
...
function getSecurityHeaders() {
	const csp = [
		...
		"img-src 'self' data: https: blob: https://www.facebook.com https://images.salambumi.xyz",
		"connect-src 'self' https://*.supabase.co https://*.googleapis.com https://*.google-analytics.com https://*.googletagmanager.com https://graph.facebook.com https://images.salambumi.xyz",
		"frame-src 'self' https://www.facebook.com https://www.google.com https://images.salambumi.xyz",
		"object-src 'none'",
		"base-uri 'self'",
		"form-action 'self'",
	...
	return {
		...
		'X-Frame-Options': 'SAMEORIGIN', // Changed from DENY to allow same-origin framing
...
async function handleImageUpload(request, env) {
	...
	try {
		...
		// --- JIKA FILE PDF, SIMPAN LANGSUNG KE R2 (Cloudflare Images hanya untuk gambar) ---
		if (ext === 'pdf') {
			console.log('📄 Processing PDF file - saving directly to R2');
			
			const pdfKey = `agreements/${propertyId}/${timestamp}-agreement.pdf`;
			
			await env.IMAGES_BUCKET.put(pdfKey, file, {
				httpMetadata: { contentType: 'application/pdf' },
			});
			
			console.log('📄 PDF saved to R2:', pdfKey);
			
			// Return R2 URL
			const pdfUrl = `${publicDomain}/${pdfKey}`;
		...
		// --- UPLOAD KE CLOUDFLARE IMAGES UNTUK OPTIMASI (HANYA UNTUK GAMBAR) ---
