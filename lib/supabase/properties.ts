...
// Client-side function for fetching properties (use in client components)
export async function getPropertiesClient(): Promise<Property[]> {
  try {
    ...
    console.error("[v0] Error in getPropertiesClient:", error)
    return []
  }
}

// Client-side function for fetching a single property
export async function getPropertyByIdClient(id: string): Promise<Property | null> {
  try {
    const supabase = getSupabaseBrowserClient()
    ...
    console.error("[v0] Error in getPropertyByIdClient:", error)
    return null
  }
}

// Client-side function for fetching properties with pagination
export async function getPropertiesClientPaginated(limit: number = 20, offset: number = 0): Promise<Property[]> {
  try {
    const supabase = getSupabaseBrowserClient()
    ...
    console.error("[v0] Error in getPropertiesClientPaginated:", error)
    return []
  }
}

// Client-side function for fetching featured properties
export async function getFeaturedPropertiesClient(): Promise<Property[]> {
  try {
    const supabase = getSupabaseBrowserClient()
    const { data, error } = await supabase
      .from("properties")
    ...
    console.error("[v0] Error in getFeaturedPropertiesClient:", error)
    return []
  }
}
